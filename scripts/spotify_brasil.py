...
# 2. Definir ruta absoluta para appbrasil.log
RUTA_LOG = os.path.join(CARPETA_LOGS, 'appbrasil.log')
...
def main():
  # IDs de playlists específicas de Brasil
  global_top_50_id = '00ZA4PMrLzLUSkGhoasI48'
  filtro_playlist_id = '2tCiwCpt3bs2x10rdNA5Tc'
  new_playlist_id = '6eRDozvQyzpWkvu1XprBh7'
  ...
  logging.info('Iniciando el proceso de actualización de playlists (Brasil)...')
  ...
  for track in top_50_global_tracks:
    ...
    if track_obj['id'] not in filtro_track_ids:
      ...
      canciones_reporte.append({
          ...
          'detalle': 'Agregada a "Top 50 Brasil New"',
      })
  ...
  # 3. Procesar reproducciones desde Last.fm en la playlist "Brasil New"
  ...
  # 5. Generar y abrir el reporte HTML en el navegador para Brasil
  generar_y_abrir_reporte('Top 50 Brasil', estadisticas, canciones_reporte)

  logging.info('El programa (Brasil) ha finalizado correctamente.')
  logging.info('----------------------------------------------------------------------------------------------------------')
